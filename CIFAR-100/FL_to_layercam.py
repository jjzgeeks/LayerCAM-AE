...
def find_outliers_autoencoder(gradcam_list, img, curr_round):
    ...
    interval = 6
    rounds_set = [list(range(1, 1 + interval)), list(range(15, 15 + interval)), list(range(29, 29 + interval))]
    p_rounds = list(np.concatenate(rounds_set))
    #if curr_round in p_rounds:
    #    for idx in range(num_clients):

            #plt.imsave(f'./client_{idx}_{curr_round}_afrae_heat.jpg', restr_gradcams[idx])
            ...
            #
            # if curr_round in list(range(49, 49+interval)):
            #     plt.imsave(f'./client_{idx}_{curr_round}_afrae_heat.jpg', restr_gradcams[idx])
    ...
    threshold = torch.mean(reconstruction_errors) + 1.0 * torch.std(reconstruction_errors)
...
def FL_to_Layer_CAM(local_weights, curr_round):
    ...
    gradcam_model = resnet50(weights=ResNet50_Weights.DEFAULT)
    #gradcam_model = regnet_y_800mf(weights=RegNet_Y_800MF_Weights.DEFAULT)
    #gradcam_model = efficientnet_b4(weights=EfficientNet_B4_Weights.DEFAULT)
    ...
    for local_weight in local_weights:
        ...
        idx += 1
    end_grad = time.time()
    print("GradCAM consumes time taken: {}".format(end_grad - start_grad))
    ...
    end_AE  = time.time()
    print("Autoencoder consumes time taken: {}".format(end_AE - start_AE))
